...
dx = Lx/Nx;
tfinal = 300; # final time, in omega_1^-1
...
xaini = 0 ;         # initial position, with respect to focus
yaini = 0 ;
...
durb = 50;
w0b = 4;
Nrefrb = np.sqrt(1-wpw1**2/4.0)
kb = 2.0*Nrefrb
zRb = np.pi*w0b**2*Nrefrb*2*np.pi/0.5;              # Rayleigh range
...
for rb,phib in cpls:   
    ...
    xbini = 0.0 # -rb*np.cos(phib/180*np.pi) ;         # initial position, with respect to focus
    ybini = 0.0 # rb*np.sin(phib/180*np.pi)  ;
    ...
    ub0 = ampb*np.exp(-1j*kb*(yyb**2)/(2*qbini))*np.exp(-(xxb)**2/(durb**2))
    ub0s.append(ub0)
    vgbs.append(vgb)
    couplings.append(Vfrs*Wfrs*0.0)

datainputs=[[maindir,cpl,ua0,ub0,vga,vgb,cvph1,cvph2,w2w1,wpw1,x,y,kxm,kym,k2xm,k2ym,dt,Es,coupling,Nt] for cpl,ub0,vgb,coupling in zip(cpls,ub0s,vgbs,couplings)]
