...
def test_focus():
    ...
    for file in fllst:
        ...
        indx2=np.where(np.abs(uaenv[int(uaenv.shape[1]/2):,indx]-np.amax(uaenv[int(uaenv.shape[1]/2):,indx])/np.sqrt(2))<1e-2)
        xmax.append(x[indx])
        dw.append(2.0*y[int(uaenv.shape[1]/2)+indx2[0][0]])

    zRa = 50.2654824574
    dwexpctd=[10*np.sqrt(1+x**2/zRa**2) for x in xmax]
